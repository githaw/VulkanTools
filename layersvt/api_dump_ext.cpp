...
static constexpr uint32_t kMaxQueriesPerCmdBuffer = 2048;
...
static std::unordered_map<void *, api_dump_ext_layer_data *> layer_data_map;

template api_dump_ext_layer_data *GetLayerDataPtr<api_dump_ext_layer_data>(
    void *data_key, std::unordered_map<void *, api_dump_ext_layer_data *> &data_map);

static void InitCommandBufferState(api_dump_ext_layer_data *dev_data, VkCommandBuffer command_buffer) {
    ...
    state.next_query = 0;
    state.subpasses.clear();
    state.warned_overflow = false;
    state.active_subpass = -1;
...
static bool ReserveQueryPair(CommandBufferState &state, SubpassRecord &record) {
    ...
    state.subpasses.push_back(record);
    return true;
}

static void DumpSubpassTimings(api_dump_ext_layer_data *dev_data, const char *wait_label, uint32_t fence_count,
                               const VkFence *fences, VkBool32 wait_all, uint64_t fence_submit_id, VkQueue queue,
                               uint64_t queue_submit_id) {
    ...
    for (auto &entry : dev_data->command_buffers) {
        ...
        if (state.subpasses.empty() || state.next_query == 0) continue;
        ...
        for (size_t i = 0; i < state.subpasses.size(); ++i) {
            const SubpassRecord &record = state.subpasses[i];
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateDevice(VkPhysicalDevice gpu, const VkDeviceCreateInfo *pCreateInfo,
    ...
    api_dump_ext_layer_data *my_device_data = GetLayerDataPtr(get_dispatch_key(*pDevice), layer_data_map);
    ...
    api_dump_ext_layer_data *instance_data = GetLayerDataPtr(get_dispatch_key(instance), layer_data_map);
...
VKAPI_ATTR VkResult VKAPI_CALL vkEnumeratePhysicalDevices(VkInstance instance, uint32_t *pPhysicalDeviceCount,
    ...
    api_dump_ext_layer_data *my_data = GetLayerDataPtr(key, layer_data_map);
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateInstance(const VkInstanceCreateInfo *pCreateInfo, const VkAllocationCallbacks *pAllocator,
    ...
    api_dump_ext_layer_data *my_data = GetLayerDataPtr(get_dispatch_key(*pInstance), layer_data_map);
...
VKAPI_ATTR VkResult VKAPI_CALL vkAllocateCommandBuffers(VkDevice device, const VkCommandBufferAllocateInfo *pAllocateInfo,
    ...
    api_dump_ext_layer_data *dev_data = GetLayerDataPtr(get_dispatch_key(device), layer_data_map);
...
VKAPI_ATTR VkResult VKAPI_CALL vkBeginCommandBuffer(VkCommandBuffer commandBuffer, const VkCommandBufferBeginInfo *pBeginInfo) {
    api_dump_ext_layer_data *dev_data = GetLayerDataPtr(get_dispatch_key(commandBuffer), layer_data_map);
    VkuDeviceDispatchTable *pTable = dev_data->device_dispatch_table;
...
VKAPI_ATTR VkResult VKAPI_CALL vkResetCommandBuffer(VkCommandBuffer commandBuffer, VkCommandBufferResetFlags flags) {
    ...
    if (result == VK_SUCCESS) {
        ...
        if (it != dev_data->command_buffers.end()) {
            ...
            it->second.subpasses.clear();
            it->second.warned_overflow = false;
            it->second.active_subpass = -1;
...
VKAPI_ATTR void VKAPI_CALL vkCmdBeginRenderPass(VkCommandBuffer commandBuffer, const VkRenderPassBeginInfo *pRenderPassBegin,
    ...
    if (ReserveQueryPair(state, record)) {
        ...
        state.active_subpass = static_cast<int32_t>(state.subpasses.size() - 1);
        state.current_renderpass = pRenderPassBegin->renderPass;
        state.current_subpass = 0;
        SubpassRecord &active = state.subpasses.back();
        active.renderpass = pRenderPassBegin->renderPass;
        active.subpass = state.current_subpass;
    } else {
        if (!state.warned_overflow) {
            fprintf(stdout, "[api_dump_ext] Query pool overflow, skip recording subpass\n");
            fflush(stdout);
...
VKAPI_ATTR void VKAPI_CALL vkCmdEndRenderPass(VkCommandBuffer commandBuffer) {
    ...
    if (state.active_subpass >= 0 && state.active_subpass < static_cast<int32_t>(state.subpasses.size())) {
        const auto &record = state.subpasses[static_cast<size_t>(state.active_subpass)];
...
VKAPI_ATTR void VKAPI_CALL vkCmdNextSubpass(VkCommandBuffer commandBuffer, VkSubpassContents contents) {
    ...
    if (ReserveQueryPair(state, record)) {
        ...
        state.active_subpass = static_cast<int32_t>(state.subpasses.size() - 1);
        SubpassRecord &active = state.subpasses.back();
        active.renderpass = state.current_renderpass;
        active.subpass = state.current_subpass;
    } else {
        state.active_subpass = -1;
    }
...
VKAPI_ATTR VkResult VKAPI_CALL vkQueueWaitIdle(VkQueue queue) {
    ...
    uint64_t queue_submit_id = 0;
    auto qit = dev_data->queue_submit_ids.find(queue);
    if (qit != dev_data->queue_submit_ids.end()) {
        queue_submit_id = qit->second;
    }
    DumpSubpassTimings(dev_data, "vkQueueWaitIdle", 0, nullptr, VK_FALSE, 0, queue, queue_submit_id);
...
VKAPI_ATTR VkResult VKAPI_CALL vkDeviceWaitIdle(VkDevice device) {
    ...
    DumpSubpassTimings(dev_data, "vkDeviceWaitIdle", 0, nullptr, VK_FALSE, 0, VK_NULL_HANDLE, 0);
...
VKAPI_ATTR VkResult VKAPI_CALL vkWaitForFences(VkDevice device, uint32_t fenceCount, const VkFence *pFences, VkBool32 waitAll,
    ...
    DumpSubpassTimings(dev_data, "vkWaitForFences", fenceCount, pFences, waitAll, fence_submit_id, fence_queue,
...
VKAPI_ATTR VkResult VKAPI_CALL vkGetPhysicalDeviceToolPropertiesEXT(VkPhysicalDevice physicalDevice, uint32_t *pToolCount,
    ...
    static const VkPhysicalDeviceToolPropertiesEXT layer_tool_props = {
        ...
        nullptr,
        "API Dump Extension Layer",
        "1",
        VK_TOOL_PURPOSE_PROFILING_BIT_EXT | VK_TOOL_PURPOSE_ADDITIONAL_FEATURES_BIT_EXT,
        "The VK_LAYER_LUNARG_api_dump_ext utility layer provides subpass timing and submit tracing to stdout.",
        "VK_LAYER_LUNARG_api_dump_ext"};
    ...
    api_dump_ext_layer_data *my_data = GetLayerDataPtr(get_dispatch_key(physicalDevice), layer_data_map);
    VkResult result =
...
#undef ADD_HOOK
    ...
    api_dump_ext_layer_data *dev_data = GetLayerDataPtr(get_dispatch_key(dev), layer_data_map);
    VkuDeviceDispatchTable *pTable = dev_data->device_dispatch_table;
